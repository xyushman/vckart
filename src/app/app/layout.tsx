...
export default function CustomerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: 'Assistant', href: '/app', icon: Mic },
    { name: 'Discover', href: '/app/discover', icon: Compass },
    { name: 'Shopping List', href: '/app/shopping-list', icon: ShoppingBag },
    ...
    <div className="flex h-screen bg-[var(--background)] text-[var(--foreground)] overflow-hidden">
      ...
      <aside className="w-64 border-r border-[var(--border)] bg-[var(--surface)] hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navItems.map(item => (
            <Link key={item.name} href={item.href} className={getLinkClasses(item.href, item.href === '/app')}>
        ...
        <div className="p-4 border-t border-[var(--border)]">
          <Link href="/app/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium hover:bg-[var(--surface-hover)] transition-colors">
            <div className="w-8 h-8 rounded-full bg-[var(--accent)]/20 flex items-center justify-center text-[var(--accent)]">
              <User className="w-4 h-4" />
            </div>
            <div>
              <p className="text-sm font-medium text-[var(--foreground)]">Alex</p>
              <p className="text-xs text-[var(--text-secondary)]">alex@example.com</p>
            </div>
          </Link>
        </div>
      </aside>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 border-t border-[var(--border)] bg-[var(--surface)] flex items-center justify-around px-2 z-50">
        {navItems.slice(0,4).map(item => (
          <Link key={item.name} href={item.href} className={getMobileClasses(item.href, item.href === '/app')}>
            <item.icon className="w-5 h-5" />
            <span className="text-[10px] font-medium">{item.name === 'Shopping List' ? 'List' : item.name}</span>
          </Link>
        ))}
        <Link href="/app/profile" className={getMobileClasses('/app/profile')}>
